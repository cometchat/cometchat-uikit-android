...
public class CometChatMessageReaction extends RelativeLayout {
    ...
    public void bindReactionsToMessage(BaseMessage baseMessage, int reactionLimit) {
        ...
        for (int i = 0; i < maxLimit; i++) {
            ...
            binding.parentView.addView(getReactionChip(reactionCount.getReaction(), reactionCount.getCount(), reactionCount.getReactedByMe(), v -> handleReactionClick(baseMessage, reactionCount), v -> handleReactionLongClick(baseMessage, reactionCount)));
        }
    ...
    /**
     * Sets the listener for reaction click events.
     *
     * @param cometchatUIKitReactionActionEvents The listener for reaction click events.
     */
    ...
    private void handleReactionClick(BaseMessage baseMessage, ReactionCount reactionCount) {
        if (cometchatUIKitReactionActionEvents != null) {
            if (reactionCount.getReactedByMe()) {
                cometchatUIKitReactionActionEvents.onReactionRemoved(getContext(), baseMessage, reactionCount.getReaction());
            } else {
                cometchatUIKitReactionActionEvents.onReactionAdded(getContext(), baseMessage, reactionCount.getReaction());
            }
        }
    }
    ...
    private boolean handleReactionLongClick(BaseMessage baseMessage, ReactionCount reactionCount) {
        if (cometchatUIKitReactionActionEvents != null) {
            cometchatUIKitReactionActionEvents.onReactionLongClick(getContext(), baseMessage, reactionCount.getReaction());
        }
        return false;
    }
    ...
    private void handleViewMoreClick(BaseMessage baseMessage) {
        if (cometchatUIKitReactionActionEvents != null) {
            cometchatUIKitReactionActionEvents.onOpenMoreReactions(getContext(), baseMessage);
        }
    }
