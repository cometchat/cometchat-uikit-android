...
public class ConversationsUtils {
        ...
        if (lastMessage == null) {
            ...
            if (textMessage.getText() != null && !textMessage.getText().isEmpty() && textMessage.getDeletedAt() == 0) {
                ...
                if (metadata != null && checkForLinks(metadata.toString())) {
                    return new LastMessageData(MessageType.LINK, Utils.getMessagePrefix(lastMessage, context), checkProfanityMessageAndDataMasking(context, textMessage));
                }
                return new LastMessageData(MessageType.DEFAULT, Utils.getMessagePrefix(lastMessage, context), checkProfanityMessageAndDataMasking(context, textMessage));
            } else {
                return new LastMessageData(MessageType.DELETED_MESSAGE, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_this_message_deleted));
            }
            ...
            if (lastMessage.getDeletedAt() == 0) {
                if (lastMessage.getType().equals(CometChatConstants.MESSAGE_TYPE_IMAGE)) {
                    ...
                    if (mediaMessage.getAttachment() != null && mediaMessage.getAttachment().toString().contains(".gif")) {
                        return new LastMessageData(MessageType.GIF, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_gif));
                    } else {
                        return new LastMessageData(MessageType.PHOTO, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_image));
                    }
                } else if (lastMessage.getType().equals(CometChatConstants.MESSAGE_TYPE_VIDEO)) {
                    return new LastMessageData(MessageType.VIDEO, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_video));
                } else if (lastMessage.getType().equals(CometChatConstants.MESSAGE_TYPE_FILE)) {
                    return new LastMessageData(MessageType.DOCUMENT, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_document));
                } else if (lastMessage.getType().equals(CometChatConstants.MESSAGE_TYPE_AUDIO)) {
                    return new LastMessageData(MessageType.AUDIO, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_audio));
                }
            } else {
            ...
            if (CallUtils.isVideoCall(call)) {
                if (CallUtils.isCallInitiatedByMe(call)) {
                    ...
                    if (call.getCallStatus().equals(UIKitConstants.CallStatusConstants.UNANSWERED) || call.getCallStatus().equals(UIKitConstants.CallStatusConstants.CANCELLED)) {
            ...
            if (lastMessage.getDeletedAt() == 0) {
                ...
                switch (lastMessage.getType()) {
                    case ExtensionConstants.ExtensionType.EXTENSION_POLL:
                        return new LastMessageData(MessageType.POLL, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_poll));
                    case ExtensionConstants.ExtensionType.STICKER:
                        return new LastMessageData(MessageType.STICKER, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_sticker));
                    case ExtensionConstants.ExtensionType.LOCATION:
                        return new LastMessageData(MessageType.LOCATION, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_location));
                    case ExtensionConstants.ExtensionType.DOCUMENT:
                        return new LastMessageData(MessageType.COLLABORATIVE_DOCUMENT, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_collaborative_document));
                    case ExtensionConstants.ExtensionType.WHITEBOARD:
                        return new LastMessageData(MessageType.COLLABORATIVE_WHITEBOARD, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_message_collaborative_whiteboard));
                    case ExtensionConstants.ExtensionType.MEETING:
                        ...
                        if (senderName.equals(context.getString(R.string.cometchat_you))) {
                            return new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, context.getString(R.string.cometchat_meeting_initiated_by_you), senderName));
                        } else {
                            return new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, context.getString(R.string.cometchat_meeting_initiated_by_others), senderName));
                        }
                    default:
                        if (lastMessage.getMetadata() != null && lastMessage.getMetadata().has("pushNotification")) {
                            try {
                                return new LastMessageData(MessageType.DEFAULT, Utils.getMessagePrefix(lastMessage, context), lastMessage.getMetadata().getString("pushNotification"));
            ...
            return new LastMessageData(MessageType.NOT_SUPPORTED, Utils.getMessagePrefix(lastMessage, context), context.getString(R.string.cometchat_this_message_type_is_not_supported));
    ...
    public static void bindConversationTailView(ConversationTailView conversationTailView, String datePattern, Conversation conversation, @StyleRes int badgeStyle, @StyleRes int dateStyle) {
    ...
    public static void bindSubtitleView(Context context, SubtitleView subtitleView, Conversation conversation, HashMap<Conversation, TypingIndicator> typingIndicatorHashMap, boolean disableTyping, boolean disableReadReceipt, List<CometChatTextFormatter> formatters, @StyleRes int conversationsItemSubtitleTextAppearance, @ColorInt int conversationsItemSubtitleTextColor, @ColorInt int conversationsItemMessageTypeIconTint, @StyleRes int conversationsReceiptStyle, @StyleRes int conversationsTypingIndicatorStyle) {
        ...
        if (!disableTyping) {
            if (!typingIndicatorHashMap.isEmpty() && typingIndicatorHashMap.containsKey(conversation)) {
                ...
                if (typingIndicator != null) {
                    try (TypedArray typedArray = context.getTheme().obtainStyledAttributes(conversationsTypingIndicatorStyle, R.styleable.CometChatTypingIndicator)) {
                        subtitleView.setTypingIndicatorTextAppearance(typedArray.getResourceId(R.styleable.CometChatTypingIndicator_cometchatTypingIndicatorTextAppearance, 0));
                        subtitleView.setTypingIndicatorTextColor(typedArray.getColor(R.styleable.CometChatTypingIndicator_cometchatTypingIndicatorTextColor, 0));
                    }
                    if (typingIndicator.getReceiverType().equalsIgnoreCase(CometChatConstants.RECEIVER_TYPE_USER)) {
                        ...
                        subtitleView.setTypingIndicatorText(typingIndicator.getSender().getName() + " " + context.getString(R.string.cometchat_is_typing));
                    }
        ...
        if (lastMessageData.messageType == MessageType.NOT_SUPPORTED || lastMessageData.messageType == MessageType.DELETED_MESSAGE) {
            ...
            SpannableString spannableString = SpannableString.valueOf(FormatterUtils.getFormattedText(context, conversation.getLastMessage(), UIKitConstants.FormattingType.CONVERSATIONS, null, lastMessageData.getLastMessage(), formatters != null && !formatters.isEmpty() ? formatters : new ArrayList<>()));
        ...
        if (action.getType().equalsIgnoreCase(CometChatConstants.ActionKeys.ACTION_TYPE_GROUP_MEMBER)) {
            switch (action.getAction()) {
                case CometChatConstants.ActionKeys.ACTION_JOINED: {
                    ...
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_GROUP_JOINED_MESSAGE, actionBy.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_LEFT: {
                    User actionBy = (User) action.getActionBy();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_GROUP_LEFT_MESSAGE, actionBy.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_KICKED: {
                    User actionBy = (User) action.getActionBy();
                    User actionOn = (User) action.getActionOn();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_MEMBER_KICKED_MESSAGE, actionBy.getName(), actionOn.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_BANNED: {
                    User actionBy = (User) action.getActionBy();
                    User actionOn = (User) action.getActionOn();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_MEMBER_BANNED_MESSAGE, actionBy.getName(), actionOn.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_UNBANNED: {
                    User actionBy = (User) action.getActionBy();
                    User actionOn = (User) action.getActionOn();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_MEMBER_UNBANNED_MESSAGE, actionBy.getName(), actionOn.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_MEMBER_ADDED: {
                    User actionBy = (User) action.getActionBy();
                    User actionOn = (User) action.getActionOn();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_MEMBER_ADDED_TO_GROUP, actionBy.getName(), actionOn.getName()));
                    break;
                }
                case CometChatConstants.ActionKeys.ACTION_SCOPE_CHANGED: {
                    User actionBy = (User) action.getActionBy();
                    User actionOn = (User) action.getActionOn();
                    lastMessageData = new LastMessageData(MessageType.DEFAULT, "", String.format(Locale.US, CometChatConstants.ActionMessages.ACTION_MEMBER_SCOPE_CHANGED, actionBy.getName(), actionOn.getName(), action.getNewScope()));
                    break;
                }
                default: {
            ...
            switch (action.getAction()) {
                ...
                case CometChatConstants.ActionKeys.ACTION_MESSAGE_DELETED:
                    lastMessageData = new LastMessageData(MessageType.DELETED_MESSAGE, "", CometChatConstants.ActionMessages.ACTION_MESSAGE_DELETED_MESSAGE);
                    break;
            }
        }
