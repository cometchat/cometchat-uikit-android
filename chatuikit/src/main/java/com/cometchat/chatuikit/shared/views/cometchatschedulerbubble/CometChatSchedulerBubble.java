...
public class CometChatSchedulerBubble extends MaterialCardView {
    ...
    private void createScheduleView(DateTimeRange dateTimeRange) {
        if (dateTimeRange != null && dateTimeRange.getFrom() != null) {
            ...
            scheduleHeaderContainer.setPadding(Utils.convertDpToPx(getContext(), 7), Utils.convertDpToPx(getContext(), 16), Utils.convertDpToPx(getContext(), 7), Utils.convertDpToPx(getContext(), 16));
        ...
        if (buttonElement.getAction() != null && !buttonElement.getAction().getActionType().isEmpty()) {
            switch (buttonElement.getAction().getActionType()) {
                case InteractiveConstants.ACTION_TYPE_API_ACTION:
                    ...
                    JSONObject jsonObject = Utils.getInteractiveRequestPayload(apiAction.getPayload(), schedulerMessage.getScheduleElement().getElementId(), schedulerMessage);
                    try {
                        ...
                        schedulerData.put(InteractiveConstants.SchedulerConstant.MEET_START_AT, sendingFormat.format(dateTimeRange.getFrom().getTime()));
                    ...
                    ApiController.getInstance().call(apiAction.getMethod(), apiAction.getUrl(), jsonObject, apiAction.getHeaders(), new ApiController.APICallback() {
                        @Override
    ...
    private void markInteract() {
        CometChat.markAsInteracted(schedulerMessage.getId(), schedulerMessage.getScheduleElement().getElementId(), new CometChat.CallbackListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                schedulerMessage.getInteractions().add(new Interaction(schedulerMessage.getScheduleElement().getElementId(), System.currentTimeMillis() / 100));
    ...
    public void createCalender() {
        changeTitleAppearanceToName();
        scheduleHeaderContainer.setPadding(Utils.convertDpToPx(getContext(), 4), Utils.convertDpToPx(getContext(), 10), Utils.convertDpToPx(getContext(), 4), Utils.convertDpToPx(getContext(), 10));
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(this.getWidth(), LinearLayout.LayoutParams.WRAP_CONTENT);
        currentView = 2;
        ...
        calender.setOnDateChangeListener((calendarView, year, month, dayOfMonth) -> {
            ...
            HashMap<String, List<DateTimeRange>> range = SchedulerUtils.getMeetingRange(fromCalender, schedulerMessage.getTimezoneCode(), schedulerMessage.getAvailability(), occupiedIcsMeetings, schedulerMessage.getDuration());
            meetSchedulerView.removeAllViews();
            timeSlot.setTime(range.get(UIKitConstants.SchedulerConstants.AVAILABLE), range.get(UIKitConstants.SchedulerConstants.OCCUPIED), schedulerMessage.getDuration(), schedulerMessage.getBufferTime());
            createSlots();
        });
        header.showInHorizontalMode();
    ...
    public void setSchedulerMessage(SchedulerMessage schedulerMessage) {
        if (schedulerMessage != null) {
            ...
            header.setTitle(schedulerMessage.getTitle() != null ? schedulerMessage.getTitle() : getContext().getString(R.string.cometchat_meet_with) + schedulerMessage.getSender().getName());
            header.setAvatar(schedulerMessage.getAvatarUrl() != null ? schedulerMessage.getAvatarUrl() : schedulerMessage.getSender().getAvatar(), schedulerMessage.getSender().getName());
            ...
            allowSenderInteraction = schedulerMessage.isAllowSenderInteraction() || schedulerMessage.getSender() == null || !schedulerMessage.getSender().getUid().equals(CometChatUIKit.getLoggedInUser().getUid());
    ...
    private void createFirstThreeSlots() {
        ...
        scheduleHeaderContainer.setPadding(Utils.convertDpToPx(getContext(), 16), Utils.convertDpToPx(getContext(), 16), Utils.convertDpToPx(getContext(), 16), Utils.convertDpToPx(getContext(), 16));
        ...
        SchedulerUtils.processMeetingAsync(schedulerMessage, occupiedIcsMeetings, result -> {
            ...
            if (loopCount > 0) {
                ...
                for (int i = 0; i < loopCount; i++) {
                    ...
                    timeSchedulerItem.setPadding(Utils.convertDpToPx(getContext(), 40), Utils.convertDpToPx(getContext(), 0), Utils.convertDpToPx(getContext(), 40), Utils.convertDpToPx(getContext(), 0));
                    int finalI = i;
            ...
            layoutParams.leftMargin = Utils.convertDpToPx(getContext(), 16);
            layoutParams.rightMargin = Utils.convertDpToPx(getContext(), 16);
            textView.setText(schedulerMessage.getDuration() + getContext().getString(R.string.cometchat_min_meeting) + " • " + TimeZone.getDefault().getDisplayName());
