...
public class CometChatFileBubble extends MaterialCardView {
    ...
    private void handleDownloadClick() {
        if (onClick == null) {
            permissionHandlerBuilder.withPermissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}).withListener(permissionResultListener).check();
        } else {
    ...
    public void setMessage(MediaMessage mediaMessage) {
        ...
        if (attachment != null) {
            ...
            @SuppressLint("SimpleDateFormat") String subTitleText = new SimpleDateFormat(dateFormatterPattern).format(mediaMessage.getSentAt() * 1000) + " • " + Utils.getFileSize(size) + " • " + attachment.getFileUrl().substring(attachment.getFileUrl().lastIndexOf(".") + 1);
            ...
            @SuppressLint("SimpleDateFormat") String subTitleText = new SimpleDateFormat(dateFormatterPattern).format(mediaMessage.getSentAt() * 1000) + " • " + Utils.getFileSize((int) mediaMessage.getFile().length()) + " • " + mediaMessage.getFile().getPath().substring(mediaMessage.getFile().getPath().lastIndexOf(".") + 1);
