...
public class SchedulerUtils {
    ...
    private static void splitMeeting(HashMap<String, List<DateTimeRange>> meetings, DateTimeRange dateTimeRange, String dateKeyStart, SimpleDateFormat dateKeyFormat) {
    ...
    public static List<DateTimeRange> getFreeIntervals(Calendar from, Calendar to, List<DateTimeRange> otherMeetings, int duration, int bufferTime) {
            ...
            while (currentTimeSlot.before(meeting.getFrom())) {
                ...
                if (endOfSlot.before(meeting.getFrom()) && !endOfSlot.after(to)) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        ...
                        isFree = otherMeetings.stream().noneMatch(m -> (finalCurrentTimeSlot.before(m.getTo()) && endOfSlot.after(m.getFrom())) // Slot
        ...
        while (currentTimeSlot.before(to)) {
            ...
            if (!endOfSlot.after(to)) {
                boolean isFree = false;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    isFree = otherMeetings.stream().noneMatch(m -> (endOfSlot.after(m.getFrom()) || endOfSlot.equals(m.getFrom())) && endOfSlot.before(m.getTo()));
                }
    ...
    public static List<DateTimeRange> getFreeTimeSlots(List<DateTimeRange> availability, List<DateTimeRange> otherMeetings, int duration, int bufferTime) {
        List<DateTimeRange> freeIntervals = new ArrayList<>();
    ...
    public static HashMap<String, List<DateTimeRange>> getMeetingRange(Calendar selectedDate, String timeZoneCode, HashMap<String, List<TimeRange>> availability, HashMap<String, List<DateTimeRange>> occupiedMeets, int duration) {
        ...
        if (!timeZoneCode.equals(TimeZone.getDefault().getID())) {
            ...
            if (!dateTimeRanges.isEmpty()) {
                ...
                if (futureFromCalender.get(Calendar.HOUR) == 0 && futureFromCalender.get(Calendar.MINUTE) == 0 && futureFromCalender.get(Calendar.SECOND) == 0) {
                    ...
                    dateTimeRanges.set(dateTimeRanges.size() - 1, new DateTimeRange(dateTimeRanges.get(dateTimeRanges.size() - 1).getFrom(), toCalenderMeeting));
                }
            }
    ...
    public static void processMeetingAsync(SchedulerMessage schedulerMessage, HashMap<String, List<DateTimeRange>> occupiedMeetings, Callback callback) {
        ExecutorService executor = Executors.newSingleThreadExecutor();
        Handler handler = new Handler(Looper.getMainLooper());
        executor.execute(() -> {
    ...
    private static List<DateTimeRange> processMeetingLogic(SchedulerMessage schedulerMessage, HashMap<String, List<DateTimeRange>> occupiedMeetings) {
        ...
        meetingStartDate.setTimeInMillis(SchedulerUtils.getDateFromString(schedulerMessage.getDateRangeStart(), "0000", schedulerMessage.getTimezoneCode()));
        Calendar meetingEndDate = Calendar.getInstance();
        meetingEndDate.setTimeInMillis(SchedulerUtils.getDateFromString(schedulerMessage.getDateRangeEnd(), "2359", schedulerMessage.getTimezoneCode()));
        ...
        if (meetingStartDate.after(todayDateCalendar)) {
            fromMeetingDate.setTimeInMillis(SchedulerUtils.getDateFromString(schedulerMessage.getDateRangeStart(), "0000", schedulerMessage.getTimezoneCode()));
        ...
        while (!breakLoop) {
            ...
            if (schedulerMessage.getAvailability() != null && schedulerMessage.getAvailability().containsKey(dayOfWeek)) {
                HashMap<String, List<DateTimeRange>> range = SchedulerUtils.getMeetingRange(fromMeetingDate, schedulerMessage.getTimezoneCode(), schedulerMessage.getAvailability(), clonedHashMap, schedulerMessage.getDuration());
                if (!range.isEmpty()) {
                    dateTimeRanges.addAll(SchedulerUtils.getFreeTimeSlots(range.get(UIKitConstants.SchedulerConstants.AVAILABLE), range.get(UIKitConstants.SchedulerConstants.OCCUPIED), schedulerMessage.getDuration(), schedulerMessage.getBufferTime()));
