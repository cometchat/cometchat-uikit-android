...
public class CometChatImageBubble extends MaterialCardView {
    ...
    private void loadBitmapIntoImageView(File file, String url) {
        ...
        builder.diskCacheStrategy(DiskCacheStrategy.DATA).placeholder(0).error(R.drawable.cometchat_image_placeholder).skipMemoryCache(false).load(file != null && file.exists() ? file : url).addListener(new RequestListener<Bitmap>() {
            @Override
    ...
    private void loadGifInToImageView(File file, String imageUrl) {
        ...
        gifDrawableRequestBuilder.diskCacheStrategy(DiskCacheStrategy.DATA).placeholder(0).error(R.drawable.cometchat_image_placeholder).skipMemoryCache(false).load(file != null && file.exists() ? file : imageUrl).addListener(new RequestListener<GifDrawable>() {
            @Override
