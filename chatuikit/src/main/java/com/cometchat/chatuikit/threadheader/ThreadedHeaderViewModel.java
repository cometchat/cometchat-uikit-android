...
public class ThreadedHeaderViewModel extends ViewModel {
    private static final String TAG = ThreadedHeaderViewModel.class.getSimpleName();
    ...
    public ThreadedHeaderViewModel() {
    ...
    private void updateReactions(ReactionEvent reactionEvent, boolean isAdded) {
        if (!hideReaction && reactionEvent.getParentMessageId() == parentMessage.getId()) {
            updateParentMessage(CometChatHelper.updateMessageWithReactionInfo(parentMessage, reactionEvent.getReaction(), isAdded ? CometChatConstants.REACTION_ADDED : CometChatConstants.REACTION_REMOVED));
        }
    }
